...
Bmax = 1; % Final magnetic field value (T)
steps = 100; % Number of steps betweem 
...
% %fprintf(FF,'INIT:REST');                   % Reinitialize FF averaging
% pause(1)
% fprintf(FF,'CALC:PAR1:SEL');
% fprintf(FF,'CALC:DATA:FDAT?');
% ff_trace = str2num(fscanf(FF));
...
gain = -100 + zeros(fpoints, length(Bz));
